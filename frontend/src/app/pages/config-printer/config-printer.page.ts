...
@Component({
  selector: 'app-config-printer',
  templateUrl: 'config-printer.page.html',
  styleUrls: ['config-printer.page.scss'],
})
export class ConfigPrinterPage implements OnInit {
  ...
  preferredPrinterName: string | null = null;

  constructor(
    zone: NgZone,
    private toastController: ToastController,
  ) {
    CapacitorThermalPrinter.addListener('discoverDevices', async ({ devices }: { devices: BluetoothDevice[] }) => {
      zone.run(() => {
  ...
  ngOnInit() {
    this.loadPreferredPrinter();
    this.processPendingSalePrint();
  }

  async connectDevice(device: BluetoothDevice) {
    try {
      await PrinterHelper.connectToPrinter(device.address);
      PrinterHelper.savePreferredPrinter(device);
      this.loadPreferredPrinter();
      this.isConnected = true;
      await this.processPendingSalePrint();
    } catch (e: unknown) {
      await this.catchPrintError(e);
    }
  }
  async startScan() {
  ...
  async printPdf() {
    if (!(await this.ensurePrinterReady())) return;
    ...
    fileInput.addEventListener('change', async () => {
      ...
      try {
        ...
        const progressToast = await this.toastController.create({
          message: `Renderizando ${totalPages} página(s)...`,
          duration: 0,
          position: 'bottom',
        });
        ...
        await PrinterHelper.printPdfBase64(base64, this.preferredPrinterAddress!);
        await this.successPrintError();
      } catch (e: unknown) {
        await this.catchPrintError(e);
      }
    });
  ...
  async printImage() {
    ...
    const fileHandler = async () => {
      ...
      try {
        await PrinterHelper.timeoutPromise(
          CapacitorThermalPrinter.begin().image(fileInput!.files![0]).write(),
          'printImage',
        );
        await this.successPrintError();
      } catch (e: unknown) {
  ...
  async printText(text: string | null | undefined) {
    if (!text) return;
    if (!(await this.ensurePrinterReady())) return;
    if (!text.endsWith('\n')) text += '\n';
    ...
    try {
      await PrinterHelper.timeoutPromise(
        CapacitorThermalPrinter.begin().text(text).write(),
        'printText',
      );
      await this.successPrintError();
    } catch (e: unknown) {
  ...
  async printDataCode(data: string | null | undefined, placement: BarcodeTextPlacement | null) {
    if (!data) return;
    if (!(await this.ensurePrinterReady())) return;
  ...
  async beep() {
    ...
    try {
      await PrinterHelper.timeoutPromise(
        CapacitorThermalPrinter.begin().beep().write(),
        'beep',
      );
      await this.successPrintError();
    } catch (e: unknown) {
  ...
  async cutPaper() {
    ...
    try {
      await PrinterHelper.timeoutPromise(
        CapacitorThermalPrinter.begin().feedCutPaper().write(),
        'cutPaper',
      );
      await this.successPrintError();
    } catch (e: unknown) {
  ...
  private async ensurePrinterReady(showNoConfigToast = true): Promise<boolean> {
    if (this.isConnected) return true;

    const savedAddress = PrinterHelper.getPreferredPrinter().address;
    if (!savedAddress) {
      if (showNoConfigToast) {
        const toast = await this.toastController.create({
          message: 'Primero selecciona una impresora.',
          duration: 1800,
          position: 'bottom',
          color: 'warning',
        });
    ...
    try {
      await PrinterHelper.connectToPrinter(savedAddress);
      this.preferredPrinterAddress = savedAddress;
      this.isConnected = true;
      return true;
    } catch (e: unknown) {
      await this.catchPrintError(e);
  ...
  private async catchPrintError(error: unknown) {
    const message = error instanceof Error ? error.message : String(error);
    const toast = await this.toastController.create({
      message: `Impresora desconectada apague y prenda su bluetooth nuevamente\n\nError: ${message}`,
      duration: 1500,
      position: 'bottom',
      color: 'danger',
    });
